  ...
  TagsInput.prototype = {
    ...
    add: function(item, dontPushVal, options) {
      ...
      if (self.itemsArray.length >= self.options.maxTags){
        $('.form-check-input:not(:checked)').attr('disabled', 'disabled');
    ...
    build: function(options) {
      ...
      self.$container.on('click', $.proxy(function(event) {
        ...
        if (self.options.addOnBlur && self.options.freeInput) {
          self.$input.on('focusout', $.proxy(function(event) {/* bootstrap-tagsinput v0.8.0*/
(function ($) {
  "use strict";
...
$('form').on('submit', function(event){
  event.preventDefault();
  var input = $('.bootstrap-tagsinput').text();
