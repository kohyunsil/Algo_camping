...
class CategoryModel:
    ...
    def rv_cleansing(self, target_col, filename='nv'):
        if filename == 'nv':
            data = self.nv_data
        else:
    ...
    def make_cat_predictor(self, target_col, label_name):
        # get tokenized data
        o_data = self.rv_cleansing('nv')
        print("Data loaded")
        data = o_data[[f'{target_col}', f'{label_name}']]
        ...
        with open(f"../models/cat_cls.pkl", "wb") as file:
            pickle.dump(model, file)
        print("Model saved!")
        return model

    def kk_apply_cat_predictor(self, target_col, make_cat_predictor=True):
        kk_cat = self.kk_sent_sep()

        if make_cat_predictor:
            load_model = self.make_cat_predictor('highlight_review', 'category')
        else:
            with open(f"../models/cat_cls.pkl", "rb") as file:
