...
class KoreaTourApi:
    ...
    def visitors_API(self, startYmd, endYmd):
        """
        region_type: metco(광역시), locgo(지자체)
        일자 YYMMDD 형태로 기입
        기간 내 일별 데이터 조회
        """
        url = f'http://api.visitkorea.or.kr/openapi/service/rest/DataLabService/locgoRegnVisitrDDList?'
        param = 'ServiceKey=' + self.secretKey + '&MobileOS=ETC&MobileApp=AppTest&numOfRows=1000000'
        detail_param = f'&startYmd={startYmd}&endYmd={endYmd}'
