...
if __name__ == '__main__':
    ...
    pa = PolarArea()
    # test a case
    test_user_answer = [1, 1, 1, 1, 1, 2, 1, 1]
    user_point_ls = up.calc_final_point(test_user_answer)
    print(user_point_ls)
    camp_point_ls = [90.97849526, 78.93215396, 62.31446128, 78.15030732, 81.44106687]
    # camp_point_ls = [78.08899086, 54.49973702, 45.45514572, 52.41632278, 0]
    match_pct = pa.matching_pct(camp_point_ls, user_point_ls)
    print(match_pct)

    # Test with the whole camp points
    user_point_df = pd.read_csv("user_polar_point_f.csv")[['comfort', 'together', 'fun', 'healing', 'clean']]
    camp_point_df = pd.read_csv("../../datas/algo_df_scale.csv")
    camp_point_df.set_index(['contentId', 'camp'], drop=True, inplace=True)
    match_pct_ls = []
    for r in range(len(camp_point_df)):
        match_pct = pa.matching_pct(camp_point_df.iloc[r].tolist(), [49.4, 59, 61.4, 73.3, 52.4])
        match_pct_ls.append(match_pct)
    print(max(match_pct_ls), min(match_pct_ls), np.mean(match_pct_ls))
