    ...
    def get_data(self):
        ...
        for link in tqdm(url_links):
            ...
            try:
                ...
                addr = self.driver.find_element_by_css_selector('#vContent > h4.chead.address.first.fblack > a.clipboardCopy').text
                lat = self.driver.find_element_by_css_selector('#vContent > p > em').text
            ...
            data = {
                "title": title,
                "addr": addr,
                "lat" : lat.split('경도')[0].strip(),
                "lon" : '경도' + lat.split('경도')[1].strip(),
                "environment" : en,
                "desc" : desc,
                "photo" : photo,
                "reviews" : reviews
        ...
        self.df.to_csv(constant.PATH + 'ogcamp.csv', index=False, encoding="utf-8-sig")
        self.driver.quit()